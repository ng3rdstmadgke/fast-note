...
if [ -z "$GITHUB_OWNER" ]; then
  echo "Error: GITHUB_OWNER は 環境変数 もしくは -o オプションで指定しなければなりません" >&2
  exit 1
fi
...
if [ "$PUSH" = true ]; then
  ...
  echo ""
  echo "Image URL: https://github.com/users/${GITHUB_OWNER}/packages/container/${IMAGE_NAME}"
else
